...
| OKR progress update | `EMPLOYEE`, authenticated users with employee profile | `POST /api/okrs/:okrId/progress`; `PATCH /api/okrs/:okrId/employee-approve` | Progress: `{ progressPercent, note? }`; approval: `{ comment? }` | Bearer | Created progress event or updated OKR approval state | Handle ownership denial, invalid progress, and `404`. |
